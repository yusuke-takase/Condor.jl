...
function tod_convolution_like_bc_new(cp, theta_tod, phi_tod, psi_tod, alpha, M_r)
    ...
    blm_s2s2 = zeros(ComplexF64,  lmax+1, 2lmax+1 + 8)
    blm_s2sm2 = zeros(ComplexF64,  lmax+1, 2lmax+1 + 8)
    blm_sm2s0 = zeros(ComplexF64,  lmax+1, 2lmax+1 + 8)
    blm_sm2s2 = zeros(ComplexF64,  lmax+1, 2lmax+1 + 8)
    blm_sm2sm2 = zeros(ComplexF64,  lmax+1, 2lmax+1 + 8)
    ...
    for l in cp.l_range[1]:cp.l_range[2]
        ...
        for i in 1:length(theta_tod[:])
            ...
            ## m < 0
            blm_s0s0[1:lmax+4] = blm_full[1,l+1,1:lmax+4]
            blm_s0s0[lmax+4+1:2lmax+1 + 8] = blm_full[1,l+1,lmax+4+1:2lmax+1 + 8]
            blm_s0s2[1:lmax+4] = blm_full[1,l+1,1:lmax+4]*e2iac
            blm_s0s2[lmax+4+1:2lmax+1 + 8] = blm_full[1,l+1,lmax+4+1:2lmax+1 + 8]*e2iac
            blm_s0sm2[1:lmax+4] = blm_full[1,l+1,1:lmax+4]*e2ia
            blm_s0sm2[lmax+4+1:2lmax+1 + 8] = blm_full[1,l+1,lmax+4+1:2lmax+1 + 8]*e2ia
            blm_s2s0[1:lmax+4] = blm_full[2,l+1,1:lmax+4]*e2iac
            blm_s2s0[lmax+4+1:2lmax+1 + 8] = blm_full[2,l+1,lmax+4+1:2lmax+1 + 8]*e2iac
            blm_s2s2[1:lmax+4] = blm_full[2,l+1,1:lmax+4]
            blm_s2s2[lmax+4+1:2lmax+1 + 8] = blm_full[2,l+1,lmax+4+1:2lmax+1 + 8]
            blm_s2sm2[1:lmax+4] = blm_full[2,l+1,1:lmax+4]*e4iac
            blm_s2sm2[lmax+4+1:2lmax+1 + 8] = blm_full[2,l+1,lmax+4+1:2lmax+1 + 8]*e4iac
            blm_sm2s0[1:lmax+4] = blm_full[3,l+1,1:lmax+4]*e2ia
            blm_sm2s0[lmax+4+1:2lmax+1 + 8] = blm_full[3,l+1,lmax+4+1:2lmax+1 + 8]*e2ia
            blm_sm2s2[1:lmax+4] = blm_full[3,l+1,1:lmax+4]*e4ia
            blm_sm2s2[lmax+4+1:2lmax+1 + 8] = blm_full[3,l+1,lmax+4+1:2lmax+1 + 8]*e4ia
            blm_sm2sm2[1:lmax+4] = blm_full[3,l+1,1:lmax+4]
            blm_sm2sm2[lmax+4+1:2lmax+1 + 8] = blm_full[3,l+1,lmax+4+1:2lmax+1 + 8]
            ...
            B0 = W*((C[1,1].*blm_s0s0[l+1,-l+cp.lmax+1+4:l+cp.lmax+1+4] .+ C[2,1].*blm_s0sm2[l+1,-l+cp.lmax+1+6:l+cp.lmax+1+6]/sqrt2 .+ C[3,1].*blm_s0s2[l+1,-l+cp.lmax+1+2:l+cp.lmax+1+2]/sqrt2).*ψ_temp)
            SB0 = S0.*conj.(B0)
            #SB0 = 0

function beam2alm(args::Dict)
    ...
    if main_present
        __main__ = args["beam_main_file_$main_format"]
    end
    ...
    if haskey(args, "healpy_convention")
         healpy_convention = args["healpy_convention"]
    end
    
    # alm = bm_alm_init(lmax, mmax, 3)
    println("Calculating multipoles up to lmax = $lmax, mmax = $mmax")
    main = bm_polar_stokesrotate(__main__, healpy_convention = healpy_convention)
